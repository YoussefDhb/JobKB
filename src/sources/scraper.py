"""SCRAPER: web-scraping enrichment (emerging occupations, skills, technologies)."""
...
# Occupational head nouns ("ML Engineer, Relevance & Personalization" -> "machine learning engineer").
...
# A well-formed skill span, rejects WordPiece fragments ("##al", "#x") the NER emits.
_CLEAN_SPAN = re.compile(r"^[A-Za-z][A-Za-z0-9+.#/&' -]*$")

# Single-token skill matches on free prose are unsafe.
...
def _clean_title(raw: str) -> str:
    """Fold a raw posting title to a bare role phrase."""
...
class ScraperSource(ExtractionSource):
    ...
    def extract(self, doc):
        """Turn one posting into {occupation, skills}."""
        ...
        for span in self._neural_spans(text):                
        ...
        # Pre-extracted skill tags (Remotive/RemoteOK/The Muse/GitHub).
    ...
    def ingest(self) -> None:
        ...
        for doc in docs:
            ...
            w = _recency_weight(doc.get("posted_at"))          
            if occ:
                title_freq[okey] += 1
        ...
        for key, freq in title_freq.items():
            ...
            # Mint only a genuine emerging role.
            if (freq >= C.SCRAPER_MIN_OCC_FREQ and key not in title_existing and key in title_skilled
                ...
                occ_sid[key] = sid
                # A truthful description from the role's most-demanded skills.
        ...
        # Mint a novel skill only from a curated `tags[]`/GitHub-topic source
        ...
        # Resolve each (title, skill) pair to entity ids
        ...
        # Keep an edge if the (occ, skill) pair recurs >= the floor (posting count)
    ...
    def _occ_index(self):
        """{match_key -> occupation entity_id} for real occupations."""
    ...
    def _skill_index(self):
        """{match_key(label) -> (skill entity_id, it_subtype)} over real KB skills."""
    ...
    def _neural_spans(self, text):
        """HF skill-span extraction. Fail-open."""
        ...
        try:                                        
            ...
            for e in nlp(text[:2000]):
                ...
                if 2 <= len(w) <= 40 and _CLEAN_SPAN.match(w):   
                    out.append(w)
            return out
        except Exception:  
            return []
